...
if __name__ == "__main__":
    ...
    model = GRUModel(args.n_features, 128, 5, args.n_features)
...
if __name__ == "__main__":
    ...
    model_log = open(
        "training/model_ckpts/%s_gru_model_zclip.log" % (args.dataset), "w"
    )
    ...
    model = GRUModel(num_features, 64, 5, num_features)
    model = model.to("cuda:0")
    model.apply(init_weights)
    ...
    for epoch in trange(100, position=0, leave=True):
        ...
        torch.save(
            ...
            "training/model_ckpts/%s_gru_model_zclip.pth" % args.dataset,
        )

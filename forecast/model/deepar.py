...
def accuracy_RMSE(mu: torch.Tensor, labels: torch.Tensor, relative=False):
    ...
    diff = torch.sum(
        torch.mul(
            (mu[zero_index] - labels[zero_index]), (mu[zero_index] - labels[zero_index])
        )
    ).item()
    if relative:
...
def accuracy_ROU(
    rou: float, samples: torch.Tensor, labels: torch.Tensor, relative=False
):
    numerator = 0
    denominator = 0
        ...
        if zero_index.numel() > 0:
            ...
            numerator += (
                ...
                * (
                    ...
                    - torch.sum(
                        (1 - rou) * abs_diff[labels[:, t][zero_index] <= rou_pred]
                    )
                ).item()
            )
...
def accuracy_ROU_(
    ...
    numerator = 2 * (
        rou * np.sum(abs_diff_1, axis=1) + (1 - rou) * np.sum(abs_diff_2, axis=1)
    )
